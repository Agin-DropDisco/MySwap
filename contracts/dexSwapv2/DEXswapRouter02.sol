...
contract DEXswapRouter02 is IDEXswapRouter02 {
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(
            token,
            to,
            IERC20DEXswap(token).balanceOf(address(this))
        );
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
    }
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ...
                : _to;
            IDEXswapPair(DEXswapLibrary.pairFor(factory, input, output))
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                (uint256 reserveInput, uint256 reserveOutput) = input == token0
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
                amountInput = IERC20DEXswap(input).balanceOf(address(pair)).sub(
                    reserveInput
                );
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint256 balanceBefore = IERC20DEXswap(path[path.length - 1]).balanceOf(
            to
        );
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20DEXswap(path[path.length - 1]).balanceOf(to).sub(
                balanceBefore
            ) >= amountOutMin,
            "DEXswapRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        uint256 amountOut = IERC20DEXswap(WETH).balanceOf(address(this));
        require(
